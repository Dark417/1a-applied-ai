...
class FakeChatService:
    ...
    async def chat(self, *, user_id: str, session_id: str | None, message: str) -> ChatResponse:
        ...
        return ChatResponse(
            ...
            events=[TraceEvent(type="tool_call", author="orchestrator", name="calculate", args={})],
        )

    async def stream(self, *, user_id: str, session_id: str | None, message: str):
        yield {"type": "tool_call", "author": "orchestrator", "name": "calculate", "args": {}}
...
def client(settings, fake_service) -> TestClient:
    ...
    return TestClient(app)  # no lifespan: the real agent tree is never built here
